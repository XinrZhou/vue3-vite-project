...
### 总结
耗时一个月，两次大修改。  
第一次，静态页面写完之后UI不符合要求，用Element-Plus进行了重构。  
第二次，项目接近尾声，基本功能实现，但代码质量差，不符合规范，再一次修改。  
期间还有一些小bug，发现后及时进行了修复。
暑假学习了Vue2，用的Vue-cli，Vuex，这次项目使用的是Vue3+pinia+Vite，因为有2的基础，所以3上手也比较快。  
学理论的过程中跟着视频学感觉还行，但独立上手做却有数不清的坑。  
大部分功能实现起来都比较容易，但在页面渲染服务器请求回来的数据时遇到了很多麻烦。  
部署用的Docker，虽然学过理论，但不太会用。最后用的老师写的file，docker-compose真的很cool。  
虽然绕了很多弯，但收获远大于每天debug的痛苦。  
老师的代码给了很多灵感，也学到了如何把代码写得更优雅、规范。  
这个月收获满满，做个小总结继续出发！  